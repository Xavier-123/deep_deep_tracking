...
def on_predict_start(predictor):
    ...
    predictor.args.tracking_config = ROOT/'boxmot'/opt.tracking_method /'configs'/(opt.tracking_method + '.yaml')
...
def run(args):
    ...
    predictor.args.imgsz = check_imgsz(predictor.args.imgsz, stride=model.model.stride, min_dim=2)  # check image size（必须被最大的 stride 整除）
    ...
    model = MultiYolo(
        model=model.predictor.model if ('v8' in str(args['yolo_model']) or 'rtdetr' in str(args['yolo_model'])) else args['yolo_model'],
    ...
    for frame_idx, batch in enumerate(predictor.dataset):
        ...
        # Inference（推理，返回三个head的预测结果，以及三个解耦头的模型输出）
        ...
        # Visualize, save, write results (前面是目标检测的常规算法，从这里开始走目标跟踪算法)
        n = len(im0s)
        for i in range(n):
...
def parse_opt():
    ...
    # # class 0 is person, 1 is bycicle, 2 is car... 79 is oven   15 is cat, 16 is dog
